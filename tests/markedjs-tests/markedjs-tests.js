...
describe('MarkedJS extended syntax', () => {
  ...
  it('should render correctly as a block when ref and name match perfectly [1.1.1 Non-text Content]', async () => {
    const expectedResult = minify('<p><a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html">1.1.1 Non-text Content</a></p>', minifyOpts)
  ...
  it('should render correctly with a title, as a block when using ref only [1.1.1]', async () => {
    const expectedResult = minify('<p><a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html" title="1.1.1 Non-text Content">1.1.1 Non-text Content</a></p>', minifyOpts)
  ...
  it('should render correctly inline when ref and name match perfectly [1.1.1 Non-text Content]', async () => {
    const expectedResult = minify('<p>this fails <a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html">1.1.1 Non-text Content</a> of WCAG 2.2</p>', minifyOpts)
    const test = minify(marked('this fails [1.1.1 Non-text Content]({wcagify}) of WCAG 2.2'), minifyOpts)
    expect(test).to.equal(expectedResult)
  })

  it('should render correctly with a title, inline when using ref only: [1.1.1]', async () => {
    const expectedResult = minify('<p>this fails <a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html" title="1.1.1 Non-text Content">1.1.1 Non-text Content</a> of WCAG 2.2</p>', minifyOpts)
    const test = minify(marked('this fails [1.1.1]({wcagify} "{title}") of WCAG 2.2'), minifyOpts)
    expect(test).to.equal(expectedResult)
  })

  it('should render correctly without a title, inline when using ref only: [1.1.1]', async () => {
    const expectedResult = minify('<p>this fails <a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html">1.1.1 Non-text Content</a> of WCAG 2.2</p>', minifyOpts)
    const test = minify(marked('this fails [1.1.1]({wcagify}) of WCAG 2.2'), minifyOpts)
    expect(test).to.equal(expectedResult)
  })

  it('should render correctly inline when ref is a match but name is close: [1.1.1 nontext content]', async () => {
    const expectedResult = minify('<p>this fails <a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html">1.1.1 Non-text Content</a> of WCAG 2.2</p>', minifyOpts)
    const test = minify(marked('this fails [1.1.1 nontext content]({wcagify}) of WCAG 2.2'), minifyOpts)
    expect(test).to.equal(expectedResult)
  })

  it('should render correctly inline when ref is a match but name is nonsense: [1.1.1 potato]', async () => {
    const expectedResult = minify('<p>this fails <a href="https://www.w3.org/WAI/WCAG22/Understanding/non-text-content.html">1.1.1 Non-text Content</a> of WCAG 2.2</p>', minifyOpts)
    const test = minify(marked('this fails [1.1.1 potato]({wcagify}) of WCAG 2.2'), minifyOpts)
    expect(test).to.equal(expectedResult)
  })
